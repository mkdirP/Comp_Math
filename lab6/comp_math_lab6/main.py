...
def Euler(choose:str,result:[],gap):
    ...
    if choose == "1":
        ...
        left = 1
        x0 = left
        print("%-10s %-10s %-10s %-10s %-10s"%("i","xi","yi","f(xi,yi)","Precise solution"))
        while round(x0,4) <= right:
            ...
            i = i + 1
            x0 = x0 + gap
        print("Accuracy:%.4f"%max(deviation))
        return result
    elif choose == "2":
        y = 0
        i = 0
        ...
        while round(x0,4) <= right:
            ...
            #     result = []
            #     gap = gap / 2
            #     print("The h is too big, so reducing it by:%.5f" % gap)
...
def Runge_Kutta(choose:str,result:[],gap):
    ...
    if choose == "1":
        ...
        left = 1
        x0 = left
        print("%-10s %-10s %-10s %-10s"%("i","xi","yi","Precise solution"))
        result = []
        deviation = []
        while round(x0,4) <= right:
            ...
            i = i + 1
            x0 = x0 + gap
        print("Accuracy:%.8f"%max(deviation))
...
def Andamc(choose,Ry,gap):
    if choose == "1":
        ...
        dievation = []
        if times < 4:
            print("The Admas method is not appropriate!")
            return []
        while i < 3:
        ...
        while times > 0:
            ...
            times = times - 1
            i = i + 1
        print("Accuracy:%.8f"%max(dievation))
    elif choose == "2":
        i = 0
        y0_3 = []
        x0_3 = [1, 1+gap, 1+2*gap, 1+3*gap]
...
func = input()
inter = input("Please enter the right interval\n> ")
right = float(inter)
h_string = input("Please input the h\n> ")
h = float(h_string)
##Эйлера
result_O = []
...
if func == "1":
    ...
    plt.plot(x_range, y_range, color="green", label="Precise solution")
elif func == "2":
